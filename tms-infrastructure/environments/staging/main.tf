...
# -----------------------------------------------------------------------------
# TMS STAGING ENVIRONMENT - COMPOSITION ROOT
# -----------------------------------------------------------------------------
# This file defines the 'staging' environment by composing reusable modules.
# It is configured to be a near-replica of the production environment, with a
# strong focus on high availability (Multi-AZ) to validate the application's
# resilience and behavior before production deployment, as per REQ-1-502.
# -----------------------------------------------------------------------------

# -----------------------------------------------------------------------------
# Module: Network
# Purpose: Provisions the foundational VPC, subnets, and routing for staging.
# -----------------------------------------------------------------------------
module "network" {
...
# Purpose: Provisions the HA Kubernetes cluster for staging.
# -----------------------------------------------------------------------------
module "eks" {
  source = "../../modules/eks"
...
# Purpose: Provisions the HA PostgreSQL 16 database.
# -----------------------------------------------------------------------------
module "rds" {
  source = "../../modules/rds"
  ...
  db_multi_az                 = var.rds_multi_az # true for staging
...
module "elasticache" {
  ...
  num_cache_nodes           = var.redis_num_cache_nodes # >= 2 for staging HA
...
module "mq" {
  ...
  broker_deployment_mode        = var.mq_deployment_mode # CLUSTER_MULTI_AZ for staging
